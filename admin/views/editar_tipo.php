<?php
  $page_title = 'Editar tipo  de vehículo';
...
if(!$tipo){
  $session->msg("d","No existe el id. del tipo del vehículo");
...
<?php
 if(isset($_POST['tipo'])){
   ...
   if(empty($errors)){
       ...
       $query   = "UPDATE tbl_tipovehiculo SET";
       $query  .=" Descripcion ='{$p_name}', Estado ='{$p_status}'";
       $query  .=" WHERE Id ='{$tipo['Id']}'";
       $result = $db->query($query);
               if($result && $db->affected_rows() === 1){
                 $session->msg('s',"El tipo de vehículo ha sido actualizado. ");
                 redirect('inicio.php?open=TipoVehiculo', false);
               } else {
                 $session->msg('d',' Lo siento, actualización falló.');
                 redirect('inicio.php?open=TipoVehiculo&id='.$tipo['Id'], false);
               }
 ...
 <!-- DataTales Example -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Editar tipo de vehículo</h6>
            </div>
            <div class="card-body">
  ...
  <div class="row">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
         <div class="col-md-12">
           <form method="post" action="editar_tipo.php?id=<?php echo (int)$tipo['Id'] ?>">
              <div class="form-group">
                <div class="input-group">
               <div class="col-md-12">
                <label>Tipo de vehículo:</label>  <input type="text" class="form-control" name="tipo-descripcion" value="<?php echo remove_junk($tipo['Descripcion']);?>">
              </div>
                  <div class="col-md-8">
                  <label>Estado:</label> 

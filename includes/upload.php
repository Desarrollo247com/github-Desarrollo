...
class  Media {
  ...
  public $insigniaPath = SITE_ROOT.DS.'..'.DS.'uploads/insignias';
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    if(!is_writable($this->$insigniaPath)){
        $this->errors[] = $this->$insigniaPath." Debe tener permisos de escritura!!!.";
        return false;
      }

    if(file_exists($this->$insigniaPath."/".$this->fileName)){
      $this->errors[] = "El archivo {$this->fileName} Realmente existe.";
      return false;
    }

    if(move_uploaded_file($this->fileTempPath,$this->$insigniaPath.'/'.$this->fileName))
    {
